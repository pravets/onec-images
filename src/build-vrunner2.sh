...
IMAGE_TAG="${registry_prefix}vrunner2:${onec_version}${CI_SUFFIX:-}"
...
DOCKER_BUILDKIT=1 docker build \
    ...
    -t "$IMAGE_TAG" \
    -f "${SCRIPT_DIR}/vrunner2/Dockerfile" \
    "${last_arg[@]}"

# Тесты (если есть)
TEST_SCRIPT="${SCRIPT_DIR}/../tests/test-vrunner2.sh"
if [[ -x "$TEST_SCRIPT" ]]; then
  ...
  echo "No tests found for vrunner2. Skipping tests."
  if [[ "$PUSH_IMAGE" == "true" ]]; then
      docker push "$IMAGE_TAG"
  else
      echo "Skipping push (PUSH_IMAGE=false)"
