...
[[ -z "${EDT_MCP_VERSION:-}" ]] && { echo "Переменная EDT_MCP_VERSION не задана" >&2; exit 1; }

if [[ -z "${DOCKER_REGISTRY_URL:-}" ]]; then
    echo "Ошибка: для сборки edt-mcp-server необходимо задать DOCKER_REGISTRY_URL (приватный реестр с образом edt)." >&2
    exit 1
fi
...
DOCKER_BUILDKIT=1 docker build \
    ...
    --build-arg EDT_MCP_VERSION="$EDT_MCP_VERSION" \
    --build-arg BUILD_DATE="$(date -u +'%Y-%m-%dT%H:%M:%SZ')" \
    --build-arg DOCKER_REGISTRY_URL="${DOCKER_REGISTRY_URL}" \
    -t "$IMAGE_TAG" \
    -f "${SCRIPT_DIR}/edt-mcp-server/Dockerfile" \
    "${SCRIPT_DIR}/edt-mcp-server"

# Run tests against the built image
if IMAGE_TAG="$IMAGE_TAG" ./tests/test-edt-mcp-server.sh; then
    if [[ "$PUSH_IMAGE" == "true" ]]; then
