...
# Resolve image tag from env or defaults (matches build-vrunner2.sh scheme)
resolve_image_tag() {
  if [[ -n "${IMAGE_TAG:-}" ]]; then
    echo "$IMAGE_TAG"
    return
  ...
  echo "${prefix}vrunner2:${ONEC_VERSION}${CI_SUFFIX:-}"
}

test_run_without_params() {
  log_header "Test :: vrunner2 run without params"
  ...
  log_success "vrunner2 run without params test passed"
}

test_help_shows_commands() {
  log_header "Test :: vrunner2 --help shows commands"
  ...
  log_success "vrunner2 --help test passed"
}

test_init_dev_ibcmd() {
  log_header "Test :: vrunner2 init-dev --ibcmd"
  ...
  log_success "vrunner2 init-dev --ibcmd test passed"
}

# Run tests
test_run_without_params || true
test_help_shows_commands || true
test_init_dev_ibcmd || true
