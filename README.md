...
- Триггер для сборки в Actions — тег вида `edt-mcp-server_ВерсияEDT_ВерсияMCP`, например `edt-mcp-server_2025.2.3_1.24.5`.
...
- Триггер для сборки в Actions — тег вида `edt-codepilot1c_ВерсияEDT_ВерсияCodePilot`, например `edt-codepilot1c_2025.2.3_0.1.7.20260301-0607`.
...
Проект поставляет два образа vanessa-runner:

- `vrunner` — актуальная версия vanessa-runner 3 (устанавливается из канала `SNAPSHOT`).
- `vrunner2` — стабильная версия vanessa-runner 2.6.1.

Оба образа создаются на базе `onec-platform` (onec-docker) и используют общие механизмы сборки.

- При сборке соответствующий скрипт (`src/build-vrunner.sh` или `src/build-vrunner2.sh`) сначала проверяет наличие базового образа локально (без префикса и с префиксом). Если образ не найден локально, скрипт попытается выполнить `docker pull` из реестра — но только если задан реальный `DOCKER_REGISTRY_URL` (в CI для fork'ов по умолчанию может использоваться безопасный префикс `local`, при котором pull не выполняется).
- Если базовый образ отсутствует и в реестре, скрипт автоматически вызовет `./src/build-onec-platform.sh` для локальной сборки базового образа `onec-platform:$ONEC_VERSION`. При включённом `PUSH_IMAGE=true` базовый образ будет также запушен в реестр.

Требования и ключевые переменные:
- `ONEC_VERSION` — версия платформы (например `8.3.27.1644`). Для tag-trigger сборок переменная вычисляется из имени тега (`vrunner_<ONEC_VERSION>` или `vrunner2_<ONEC_VERSION>`).
...
```bash
# собрать vrunner на vanessa-runner 3
PUSH_IMAGE=false ONEC_VERSION=8.3.27.1644 ./src/build-vrunner.sh

# собрать vrunner2 на vanessa-runner 2.6.1
PUSH_IMAGE=false ONEC_VERSION=8.3.27.1644 ./src/build-vrunner2.sh
```

Сборка через GitHub Actions по тегу (пример):

1. Создайте нужный тег и запушьте его в репозиторий:
